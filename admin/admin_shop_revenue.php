...
<body class="d-flex flex-column h-100 bg-white">
    ...
    <div class="container px-5 py-4" id="shop-body">
        ...
        <div class="container row row-cols-6 row-cols-md-12 g-5 pt-4 mb-4" id="shop-header">
            <div class="rounded-25 col-6 col-md-4" id="shop-img" style="background: url(<?php echo (is_null($shop_row["s_pic"]) ? "'../img/default.png'" : "'../img/{$shop_row['s_pic']}'"); ?>) center; height: 225px; background-size: cover; background-repeat: no-repeat; object-fit:fill; background-position: center;"></div>
            <div class="col-6 col-md-8">
                <h1 class="display-5 strong"><?php echo $shop_row["s_name"];?></h1>
                <ul class="list-unstyled">
                    <li class="">Telephone number: <?php echo "(+61) ".$shop_row["s_phoneno"];?></li> <!-- Adjusted phone number to Australian format -->
                </ul>
            ...
            <form method="GET" action="admin_shop_report.php" class="form-floating">
                ...
                <div class="form-check">
                    ...
                    <label class="form-check-label" for="rev_mode1">
                        Today<br />(<?php echo date('j F, Y');?>) <!-- Adjusted to Australian date format -->
                    </label>
                </div>
                <div class="form-check">
                    ...
                    <label class="form-check-label" for="rev_mode2">
                        Yesterday<br />(<?php echo (new Datetime()) -> sub(new DateInterval("P1D")) -> format('j F, Y');?>) <!-- Adjusted to Australian date format -->
                    </label>
                </div>
                <div class="form-check">
                    ...
                    <label class="form-check-label" for="rev_mode3">
                        ...
                        $week_start = (new Datetime($weekrange["start"])) -> format('j F, Y');
                        $week_end = (new Datetime($weekrange["end"])) -> format('j F, Y');
                ...
                <div class="form-check">
                    ...
                    <label class="form-check-label" for="rev_mode4">
                        ...
                        $month_start = (new Datetime($monthrange["start"])) -> format('j F, Y');
                        $month_end = (new Datetime($monthrange["end"])) -> format('j F, Y');
